...
# vcvarsall.bat writes to STDERR on this machine (a missing vswhere.exe) while still succeeding.
# PowerShell turns native stderr into a NativeCommandError under ErrorActionPreference=Stop, so
# without 2>nul the script fails on noise from a command that worked.
cmd /c "`"$vcvars`" x86 >nul 2>nul && cl /nologo /EHsc /W4 /Fe:`"$exe`" /Fo:`"$obj`" `"$src`" >nul && `"$exe`""
